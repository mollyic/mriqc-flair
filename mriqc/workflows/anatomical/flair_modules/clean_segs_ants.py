...
def clean_tissue_segs(name='clean_segs', padding=10):
    ...
    inputnode = Node(
        niu.IdentityInterface(fields=['segmentation', 'modality', 'pvms']), name='inputnode'
    )

    outputnode = Node(
        niu.IdentityInterface(fields=['out_segs_proc', 'out_segm', 'out_pvms']), name='outputnode'
    )
    ...
    pad_segm = Node(ImageMath(operation='PadImage', op2=f'{padding}'), name='segm_pad')

    # 1. Split segmentation in binary masks
    segm_binarise = Node(
        Function(function=_binarise_seg_labels, output_names=['out_csf', 'out_gm', 'out_wm']),
    ...
    wf.connect(
        [
            ...
            (inputnode, mask_pvms, [('modality', 'modality'), ('pvms', 'pvm')]),
...
def _binarise_seg_labels(in_segm, modality):
    ...
    for tissue, label in model.items():
        ...
        out_file = fname_presuffix(
            in_segm, suffix=f'_class-{label}_tissue-{tissue}', newpath=getcwd()
        )
        out_mask.to_filename(out_file)
        out_files[tissue] = out_file

    return (out_files.get('csf'), out_files.get('gm'), out_files.get('wm'))
...
def _tissue_wf(tissue, padding=10):
    ...
    fill_holes = Node(
        ImageMath(operation='FillHoles', op2='2', copy_header=True), name='fill_holes'
    )
    multiply_imgs = Node(
        MultiplyImages(
            dimension=3, output_product_image=f'seg-{tissue}_multiply-getLCCxfill_holes.nii.gz'
        ),
        name='multiply_imgs',
    )
    ...
    def _get_idx(file):
        ...
        return int(re.search(r'class-(\d+)', file).group(1))
...
def _rebuild_segmentation():
    ...
    outputnode = Node(
        niu.IdentityInterface(fields=['sumd_tissues', 'proc_seg_lst']), name='outputnode'
    )
    ...
    add_gm_csf = Node(
        ImageMath(operation='addtozero', output_image='%s_addcsf.nii.gz'), name='add_gm_csf'
    )
    add_all = Node(
        ImageMath(operation='addtozero', output_image='segment_processed.nii.gz'), name='add_all'
    )
    ...
    clean_wm = Node(
        MultiplyImages(dimension=3, output_product_image='seg-wm_multiply.nii.gz'), name='clean_wm'
    )
...
def _mask_pvms(pvm, modality, seg_lst):
    ...
    nb.Nifti1Image(pvm_maskd.astype(np.float32), pvm_nii.affine, pvm_nii.header).to_filename(
        out_pvm_msk
    )
