...
def init_atropos_wf(
    name='atropos_wf', use_random_seed=True, omp_nthreads=None, mem_gb=3.0, padding=10
):
    """
    ...
    # inputnode = pe.Node(niu.IdentityInterface(fields=["in_corrected", "in_mask", "modality"]), name="inputnode")

    inputnode = pe.Node(
        niu.IdentityInterface(fields=['classified_image', 'modality', 'posteriors']),
        name='inputnode',
    )

    outputnode = pe.Node(
        niu.IdentityInterface(
            fields=['out_segs_proc', 'out_segm', 'out_pvms', 'out_eroded']),
        name='outputnode',
    )

    # Morphological dilation of brainmask, radius=2
    dil_bmsk = pe.Node(ImageMath(operation='MD', op2='2', copy_header=True), name='bmask_dilate')
    # Get largest connected component
    bmsk_lcc = pe.Node(ImageMath(operation='GetLargestComponent', copy_header=True), name='bmask_lcc')
...
def _rebuild_segmentation():
    ...
    """
    inputnode = pe.Node(
        niu.IdentityInterface(fields=['csf', 'gm', 'wm']), 
        name='inputnode')
    outputnode = pe.Node(
        niu.IdentityInterface(fields=['sumd_tissues', 'proc_seg_lst']), 
        name = 'outputnode')
    ...
    add_gm_csf = pe.Node(
        ImageMath(operation='addtozero', output_image='%s_addcsf.nii.gz'), 
        name='add_gm_csf'
    )

    add_all = pe.Node(
        ImageMath(operation='addtozero', output_image='segment_processed.nii.gz'),
        name='add_all'
    )

    merge_tpms = pe.Node(
        niu.Function(function=_merged, output_names=['proc_seg_lst']), 
        name='merge_tpms'
    )

    bin_gmcsf = pe.Node(
        ThresholdImage(dimension=3, th_low=0.01, th_high=1e7, inside_value=0, outside_value=1),
        name='bin_gmcsf',
    )
    ...
    clean_wm = pe.Node(
        MultiplyImages(dimension=3, output_product_image=f'seg-wm_multiply.nii.gz'),
        name='clean_wm',
    )

    wf = pe.Workflow(name ='segm_sum')
    wf.connect([
...
def _mask_pvms(pvm, modality, seg_lst):
    ...
    out_pvm_msk = fname_presuffix(
        pvm, suffix=f'_class-{idx}_tissue-{tissue.upper()}_mskseg', newpath=getcwd()
    )
    nb.Nifti1Image(
        pvm_maskd.astype(np.float32), pvm_nii.affine, pvm_nii.header).to_filename(out_pvm_msk)
    eroded = out_pvm_msk
    ...
    if modality == 'T2w' and tissue == 'wm':
        ...
        eroded_data = binary_erosion(
            segmask_bin,
            structure=generate_binary_structure(3, connectivity=connect),
        ...
        out_t2_msk = fname_presuffix(
            pvm, suffix=f'_class-{idx}_tissue-{tissue.upper()}_mskseg_getLCC', newpath=getcwd()
        )
        print('T2w')
        print(f'\t *{tissue}:')
        print(f'\t *{out_t2_msk}:')
        ...
        nb.Nifti1Image(
            pvm_maskd_lcc.astype(np.float32), pvm_nii.affine, pvm_nii.header
        ).to_filename(out_t2_msk)
        eroded = out_t2_msk
...
def _tissue_wf(tissue, padding=10):
    ...
    outputnode = pe.Node(
        niu.IdentityInterface(fields=['out_tissue', 'erode_csf']), 
        name='outputnode'
    )

    lcc = pe.Node(ImageMath(
        operation='GetLargestComponent', 
        copy_header=True),
        name ='getLCC'
    )
    fill_holes = pe.Node(
        ImageMath(operation='FillHoles', op2='2', copy_header=True), 
        name='fill_holes',
    )
    multiply_imgs =pe.Node(
        MultiplyImages(
            dimension=3, 
            output_product_image=f'seg-{tissue}_multiply-getLCCxfill_holes.nii.gz'
            ),
        name='multiply_imgs',
    )
    morph_erode = pe.Node(
        ImageMath(operation='ME', op2='0.05', copy_header=True), 
        name='morph_erode'
    )
    reindex_tissue = pe.Node(
        MultiplyImages(dimension=3, output_product_image=f'seg-{tissue}_processed.nii.gz'),
        name='reindex_tissue',
    )
    depad = pe.Node(
        ImageMath(operation='PadImage', op2='-%d' % padding), 
        name='depad'
    )

    workflow = pe.Workflow(name=f'tissue-{tissue}_wf')
...
# atropos_test = pe.Workflow((name= 'atropos_test', base_dir = 'work_atropos/')

# fielders = ["in_corrected", 'modality', 'in_mask']
# inputnode = pe.Node(niu.IdentityInterface(fields=fielders), name="inputnode")
